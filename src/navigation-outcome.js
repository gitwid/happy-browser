(function attachHappyNavigationOutcome(globalScope) {
  ...
  function capturePageSnapshot(documentRef, locationRef, getMediaSignature) {
    ...
    return {
      ...
      bodyTextLength: documentObject.body
        ? String(documentObject.body.innerText || documentObject.body.textContent || "").length
        : 0,
      activeElementSignature: active
        ? `${active.tagName}:${active.id}:${active.className}:${active.getAttribute("aria-current") || ""}`
        : "",
      // Visible media changing counts as advancement (galleries/carousels that keep the
      // same URL and body length). content.js injects the viewport-aware signature fn.
      mediaSignature: typeof getMediaSignature === "function" ? getMediaSignature() : ""
    };
  }
  ...
  function createOutcomeMemory(options = {}) {
    ...
    const getMediaSignature = options.getMediaSignature;
    const failedSelectors = new Map();
    let preNavigationSnapshot = null;

    function captureSnapshot() {
      return capturePageSnapshot(documentRef, locationRef, getMediaSignature);
    }
